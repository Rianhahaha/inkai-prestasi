...
export default async function DetailAtletPage({ params, searchParams }: PageProps) {
  const resolvedParams = await params
  const resolvedSearchParams = await searchParams
  const athleteId = resolvedParams.id
  const payload = await getPayload({ config })
  const modalAction = resolvedSearchParams.action // 'tambah' atau 'edit'
      ...
      <>
        ...
        <div className="flex flex-col gap-6 w-full">
          ...
          <div className="flex items-center justify-between">
            ...
            <Link
              href={`/admin-dashboard/data-atlet/${athleteId}?action=edit`}
              className="bg-[#3b82f6] hover:bg-blue-600 text-white px-6 py-2.5 rounded-xl font-bold text-sm transition-colors cursor-pointer"
            >
              Edit Profil →
            </Link>
          </div>
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-10">
              <div className="flex flex-col gap-1">
                ...
                <div className="font-bold text-slate-800">
                  {user.tempatLahir || '-'},{' '}
                  {user.tanggalLahir ? formatDate(user.tanggalLahir) : '-'}
                </div>
              </div>
